...
-- 예) 2014년 07월 10일	
select min(hire_date) as '가장 오래 근속한 직원의 입사일' from employees;
...
select min(TIMESTAMPDIFF(YEAR, birth_date, NOW())) AS '최고 어린 사원',max(TIMESTAMPDIFF(YEAR, birth_date, NOW())) AS '최 연장자' from employees;
